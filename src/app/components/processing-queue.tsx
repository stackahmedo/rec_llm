  ...
  if (sortedJobs.length === 0) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-muted-foreground gap-2">
        ...
        <div className="text-xs">No files in queue</div>
        <div className="text-[10px]">Add audio files using the toolbar above</div>
      </div>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="h-7 flex items-center px-3 border-b bg-muted/10 shrink-0">
        <span className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium">Processing Queue</span>
        <span className="text-[10px] text-muted-foreground ml-auto font-mono">{sortedJobs.length} files</span>
      </div>
